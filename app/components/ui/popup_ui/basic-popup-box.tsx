...
export default function BasicPopupBox({children, type = "warm", className}: {children: React.ReactNode, type?: "warm" | "cold", className?: string}) {
  ...
  return (
    <div className="relative w-fit text-white">
      {/* 그림자 컨테이너 - 박스 뒤쪽에 별도 배치 */}
      <div className="absolute inset-0 shadow-2xl rounded-[32px]"/>
      
      {/* 메인 박스 - 그림자와 완전 분리된 별도 컨테이너 */}
      <div className="relative rounded-[32px] overflow-hidden bg-black/5 z-10">  
      {/* 그라데이션 */}
      <p className={cn(
